...
name: cso
description: |
  Chief Security Officer mode. Infrastructure-first security audit: secrets archaeology, dependency supply chain, CI/CD pipeline security, LLM/AI security, plus OWASP Top 10 and STRIDE threat modeling. Two modes: daily (zero-noise, 8/10 confidence gate) and comprehensive (monthly deep scan, 2/10 bar). Trend tracking across audit runs. Loaded by /security as the infrastructure-audit phase.
...
If you haven't already this session: set `SKILL_NAME=cso` and read `~/.skillshare/runtime/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
