...
_PROACTIVE=$(~/.skillshare/runtime/gstack/bin/gstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED="yes"
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(~/.skillshare/runtime/gstack/bin/gstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(~/.skillshare/runtime/gstack/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN="yes"
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(~/.skillshare/runtime/gstack/bin/gstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED="yes"
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(~/.skillshare/runtime/gstack/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(~/.skillshare/runtime/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "~/.skillshare/runtime/gstack/bin/gstack-telemetry-log" ]; then
      ~/.skillshare/runtime/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(~/.skillshare/runtime/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${GSTACK_HOME:-$HOME/.gstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ~/.skillshare/runtime/gstack/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
~/.skillshare/runtime/gstack/bin/gstack-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f AGENTS.md ] && grep -q "## Skill routing" AGENTS.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(~/.skillshare/runtime/gstack/bin/gstack-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d "$HOME/.skillshare/runtime/gstack" ] && [ ! -L "$HOME/.skillshare/runtime/gstack" ]; then
  if [ -f "$HOME/.skillshare/runtime/gstack/VERSION" ] || [ -d "$HOME/.skillshare/runtime/gstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_GSTACK: $_VENDORED"
echo "MODEL_OVERLAY: Codex"
_CHECKPOINT_MODE=$(~/.skillshare/runtime/gstack/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(~/.skillshare/runtime/gstack/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `~/.skillshare/runtime/gstack/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.skillshare/runtime/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `~/.skillshare/runtime/gstack/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `~/.skillshare/runtime/gstack/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `~/.skillshare/runtime/gstack/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `~/.skillshare/runtime/gstack/bin/gstack-config set explain_level terse`.
...
If A: run `~/.skillshare/runtime/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `~/.skillshare/runtime/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `~/.skillshare/runtime/gstack/bin/gstack-config set telemetry off`
...
If A: run `~/.skillshare/runtime/gstack/bin/gstack-config set proactive true`
If B: run `~/.skillshare/runtime/gstack/bin/gstack-config set proactive false`
...
If B: run `~/.skillshare/runtime/gstack/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via AskUserQuestion unless `~/.gstack/.vendoring-warned-$SLUG` exists:

> This project has gstack vendored in `$HOME/.skillshare/runtime/gstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r $HOME/.skillshare/runtime/gstack/`
2. Run `echo '$HOME/.skillshare/runtime/gstack/' >> .gitignore`
3. Run `~/.skillshare/runtime/gstack/bin/gstack-team-init required` (or `optional`)
4. Run `git add .Codex/ .gitignore AGENTS.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `re-run the Skillshare installer on each machine`"
...
_BRAIN_SYNC_BIN="~/.skillshare/runtime/gstack/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="~/.skillshare/runtime/gstack/bin/gstack-config"
...
```bash
"~/.skillshare/runtime/gstack/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
"~/.skillshare/runtime/gstack/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$(~/.skillshare/runtime/gstack/bin/gstack-slug 2>/dev/null)"
...
Curated jargon list lives at `~/.skillshare/runtime/gstack/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `~/.skillshare/runtime/gstack/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-question-log '{"skill":"review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `~/.skillshare/runtime/gstack/ETHOS.md`.
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
~/.skillshare/runtime/gstack/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ~/.skillshare/runtime/gstack/bin/gstack-telemetry-log ]; then
  ~/.skillshare/runtime/gstack/bin/gstack-telemetry-log \
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-learnings-log '{
  "type": "pitfall",
...
```bash
_CROSS_PROJ=$(~/.skillshare/runtime/gstack/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.skillshare/runtime/gstack/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.skillshare/runtime/gstack/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.skillshare/runtime/gstack/bin/gstack-config set cross_project_learnings true`
If B: run `~/.skillshare/runtime/gstack/bin/gstack-config set cross_project_learnings false`
...
```bash
source <(~/.skillshare/runtime/gstack/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.skillshare/runtime/gstack/review/specialists/testing.md`
2. **Maintainability** — read `~/.skillshare/runtime/gstack/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.skillshare/runtime/gstack/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.skillshare/runtime/gstack/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.skillshare/runtime/gstack/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.skillshare/runtime/gstack/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.skillshare/runtime/gstack/review/design-checklist.md`
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.skillshare/runtime/gstack/review/specialists/red-team.md`
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-read
```
...
OLD_CFG=$(~/.skillshare/runtime/gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
