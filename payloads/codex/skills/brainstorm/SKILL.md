...
name: brainstorm
description: |
  Brainstorm a new idea or side project before any code exists. Two modes: startup mode (six forcing questions interrogating demand reality, status quo, narrowest wedge, observation, future-fit) and builder mode (design-thinking for side projects, hackathons, learning, open source). Saves a design doc, then (once approved and the architecture is concrete) optionally scaffolds a non-functional skeleton repo from it. Use when the user says "brainstorm this", "I have an idea", "help me think through this", "is this worth building", or describes a new product idea with no code yet. Proactively invoke (do not answer directly) when the user describes a new product concept or asks if something is worth building.
...
If you haven't already this session: set `SKILL_NAME=brainstorm` and read `~/.codex/skills/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
...
B=""
[ -n "$_ROOT" ] && [ -x "$HOME/.skillshare/runtime/gstack/browse/dist/browse" ] && B="$HOME/.skillshare/runtime/gstack/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.skillshare/runtime/gstack/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
2. Run: `~/.skillshare/runtime/gstack/build-shareable.sh`
...
**HARD GATE (through Phase 5):** Until the design doc is APPROVED, do NOT invoke any implementation skill, write any code, or scaffold anything. Through approval, your only output is a design document. The ONE exception is **Phase 5.5**: after the doc is APPROVED *and* it names a concrete architecture, you may scaffold a **non-functional skeleton repo** — directory tree plus stub files (docstrings, signatures, `TODO` / `NotImplementedError` placeholders). Never real implementation logic. See Phase 5.5.
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$HOME/.skillshare/runtime/gstack/design/dist/design" ] && D="$HOME/.skillshare/runtime/gstack/design/dist/design"
[ -z "$D" ] && D="$HOME/.skillshare/runtime/gstack/design/dist/design"
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
```bash
eval "$(~/.skillshare/runtime/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
- A) Approve — mark Status: APPROVED, then proceed to Phase 5.5 (skeleton scaffold) and handoff
...
## Phase 5.5: Skeleton Repo Scaffold

Once the design doc is APPROVED **and** it names a concrete architecture (a chosen approach
plus a repo layout, component list, or named stack), offer to scaffold a **non-functional
skeleton repo** so the build has a frame to fill in. This is the only point in the skill
where writing project files is allowed, and even here you write stubs, never working logic.

**Skip this phase entirely (go straight to Phase 6) if:**
- No tech was chosen — a pure-strategy / idea-stage doc with no stack has nothing to
  scaffold. Say "No architecture chosen yet, so nothing to scaffold" and continue.
- The target already has substantial source (more than README/config). Do not scaffold over
  an existing codebase; say so and continue.

### Step 1: Offer (gate)

Use AskUserQuestion:
> The design is approved. Want me to scaffold a skeleton repo now — the directory tree and
> stub files from the architecture, with docstrings and `TODO` markers but no working code?
> You fill in the logic next.

Options:
- A) Yes, scaffold the skeleton (recommended)
- B) No, just the design doc for now

If B: skip to Phase 6.

### Step 2: Locate target + init/branch

```bash
TARGET=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$TARGET"
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git init -b main >/dev/null 2>&1 && echo "Initialized empty git repo at $TARGET"
fi
echo "TARGET: $TARGET"; echo "BRANCH: $(git branch --show-current 2>/dev/null)"; ls -A
```

If this is a **pre-existing** repo sitting on its default branch (`main`/`master`), create a
feature branch first (`git checkout -b scaffold/<slug>`) before writing anything. A brand-new
repo you just `git init`-ed stays on `main` — the scaffold IS its first commit.

### Step 3: Build the file manifest from the design doc

- Use the design doc's **Repo layout** / **Architecture** section as the source of truth for
  directories and files. If there's no explicit layout, synthesize a minimal one from the
  chosen approach, the component list, and the Distribution Plan.
- For each file, capture its one-line responsibility (the component it implements) from the doc.

### Step 4: Write stub files — NON-FUNCTIONAL

Each stub contains, in the project's language:
- A top-of-file docstring/comment stating the file's responsibility, condensed from the design.
- The interfaces the design already specified (class / function / route signatures,
  Protocol or type definitions) with bodies that are `pass`, `raise NotImplementedError("…")`,
  or `TODO:` comments — never real logic.
- A `# TODO (build order step N): …` pointer mapping the file to the design's Next Steps /
  build order, so the user knows what to implement first.

Do NOT implement parsing, network calls, business rules, persistence, or auth. Signatures
and docstrings only. The skeleton must import/compile cleanly and do nothing.

### Step 5: Write supporting files (only if absent — never overwrite)

- `.gitignore` — language-appropriate (Python: `__pycache__/`, `.venv/`, `.env`, local data
  dirs and `*.sqlite`/`*.db`).
- `README.md` — title, one-paragraph problem statement, the stack, and the build order, from
  the design doc.
- Dependency manifest — `requirements.txt` / `pyproject.toml` / `package.json` / `Cargo.toml`
  etc., listing the deps named in the design (unpinned is fine; the user pins later).
- `.env.example` — every env var the design references (names + comments, no secrets).
- One stub test file for build-order step 1 (e.g. a parser fixture test) with a `TODO` body,
  so the test harness exists from the start.

### Step 6: Commit and report

Stage only the files you created (a brand-new repo can use `git add -A`; in a pre-existing
repo with unrelated untracked files, add your scaffold files individually).

```bash
git add -A   # or the explicit list of scaffolded files in a pre-existing repo
git -c user.name="$(git config user.name || echo gstack)" \
    -c user.email="$(git config user.email || echo gstack@local)" \
    commit -m "chore: scaffold skeleton from approved design

Non-functional stubs generated by /brainstorm. See the design doc for the build order.

Co-Authored-By: Codex <noreply@anthropic.com>" >/dev/null 2>&1
git log --oneline -1
```

Then print the created tree and the first concrete action from the build order (e.g.
"Start with: `app/sources/njmvc.py` parse + the fixture test"). Reference the design doc path,
and note that `/autoplan` or `/plan-eng-review` can review the plan before the user fills in
the stubs.

**Then proceed to Phase 6 (Handoff).**
...
```bash
PROFILE=$(~/.skillshare/runtime/gstack/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
```bash
eval "$(~/.skillshare/runtime/gstack/bin/gstack-paths)"
...
```bash
eval "$(~/.skillshare/runtime/gstack/bin/gstack-slug 2>/dev/null || true)"
~/.skillshare/runtime/gstack/bin/gstack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **No implementation logic.** This skill produces a design doc and, only after it is APPROVED, an optional non-functional skeleton repo (stub files, no working code — Phase 5.5). Never write real logic, and never scaffold during the questioning/premise/alternatives phases (Phases 1-4).
