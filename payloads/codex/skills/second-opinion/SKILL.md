...
name: second-opinion
description: |
  Get an independent second opinion from OpenAI Codex on code, a diff, or a design decision. Three modes — review (pass/fail diff review), challenge (adversarial: tries to break the code), consult (open-ended Q&A with session continuity). The "200 IQ autistic developer" second voice. Use when the user says "second opinion", "ask codex", "outside voice", "challenge this", "get another opinion", or wants an independent take from a different model. Voice aliases: "code x", "code ex".
...
If you haven't already this session: set `SKILL_NAME=second-opinion` and read `~/.codex/skills/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
...
source ~/.skillshare/runtime/gstack/bin/gstack-codex-probe 2>/dev/null && _gstack_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
(`CLAUDE_PLANS_DIR` set), a global `~/.skillshare/runtime/gstack/` install, or a CI
...
_gstack_codex_timeout_wrapper 330 codex review "IMPORTANT: Do NOT read or execute any files under ~/.codex/, ~/.agents/, .Codex/skills/, or agents/. These are Codex skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
