...
**Root cause:** Environment variable not set or Codex needs restart

**Solution:**
1. Verify key is in `~/.codex/settings.json`:
   ```bash
   grep OPENAI_API_KEY ~/.codex/settings.json
   ```
2. Restart Codex (close and reopen)
3. Try again
...
3. Update `~/.codex/settings.json` with new key
4. Restart Codex
...
if not api_key:
    ...
    print("   → Add to ~/.codex/settings.json under 'env'")
    sys.exit(1)
...
- If Codex restarts, the environment variable persists (stored in settings.json)
