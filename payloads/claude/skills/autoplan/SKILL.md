...
name: autoplan
description: |
  Auto-review pipeline — reads the full CEO, design, eng, and DX review skills from disk and runs them sequentially with auto-decisions using 6 decision principles. Surfaces taste calls (close approaches, borderline scope, codex disagreements) at a final approval gate. One command, fully-reviewed plan out. Use when the user has a plan file and wants the full review gauntlet without 15-30 intermediate questions — "auto review", "autoplan", "run all reviews", "review this plan automatically", "make the decisions for me".
...
If you haven't already this session: set `SKILL_NAME=autoplan` and read `~/.claude/skills/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
...
**This offer is skippable-by-evidence, not just skippable-by-answer.** Its whole
purpose is to make sure *some* real planning happened before the review gauntlet
runs — not to force the specific ritual of running `/brainstorm` as a separate
step. If planning has clearly already happened (a fleshed-out plan file, or this
same conversation already worked through the problem/alternatives/premises before
invoking `/autoplan`), do NOT ask. Proceed straight to Step 0 of the review pipeline
silently. Only ask when there's a real gap.

**Check for evidence of prior planning, in order:**

1. **No design doc found**, but the plan file itself is already substantive —
   it has more than a title/one-liner, and covers at least two of: a problem
   statement, considered alternatives, explicit premises/assumptions, scope
   boundaries. → Evidence of planning exists. Skip the offer, proceed silently.
2. **No design doc found**, and this conversation's own history already shows
   the user and Claude working through the problem (challenging premises,
   weighing approaches, narrowing scope) before `/autoplan` was invoked, even
   without a formal design doc artifact. → Skip the offer, proceed silently.
3. **No design doc found**, the plan file is a bare stub (a few bullet points,
   no alternatives or premises considered), AND nothing in this conversation
   suggests planning happened yet. → This is the only case that warrants asking.

Only in case 3, offer the prerequisite skill via AskUserQuestion:

> "This plan looks like a first draft — no design doc, and we haven't worked
> through the problem together yet. `/brainstorm` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review
> much sharper input to work with. Takes about 10 minutes. The design doc is
> per-feature, not per-product — it captures the thinking behind this specific
> change."
...
If they skip: "No worries — standard review." Then proceed normally. Do not
...
Read the `/brainstorm` skill file at `~/.claude/skills/brainstorm/SKILL.md` using the Read tool.

**If unreadable:** Skip with "Could not load /brainstorm — skipping." and continue.

Follow its instructions from top to bottom, **skipping these sections if present**
(already handled by the parent skill):
- Preamble (run first)
...
- `~/.skillshare/runtime/gstack/plan-ceo-review/SKILL.md`
- `~/.skillshare/runtime/gstack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.skillshare/runtime/gstack/plan-eng-review/SKILL.md`
- `~/.skillshare/runtime/gstack/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(~/.skillshare/runtime/gstack/bin/gstack-config get telemetry 2>/dev/null || echo off)
source ~/.skillshare/runtime/gstack/bin/gstack-codex-probe
...
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.skillshare/runtime/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
