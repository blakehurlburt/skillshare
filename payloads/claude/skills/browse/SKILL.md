...
name: browse
description: |
  Fast headless browser for testing and dogfooding live web apps. Navigate URLs, interact with elements, verify page state, diff before/after, take annotated screenshots, check responsive layouts, test forms and uploads, handle dialogs, assert element states. ~100ms per command. Use when the user says "open in browser", "test the site", "take a screenshot", "dogfood this", "verify the deployment", "check the staging URL", or needs evidence for a bug report.
...
If you haven't already this session: set `SKILL_NAME=browse` and read `~/.claude/skills/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
