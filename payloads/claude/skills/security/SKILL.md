...
5. **Offer to fix.** Ask if the user wants Claude to apply fixes now, file them as TODOs, or just review.
...
- VibeSec lives at `~/.skillshare/runtime/lib/vibesec/` (moved out of `~/.claude/skills/` so it stops auto-firing as a separate skill). The `/cso` SKILL.md still lives at `~/.skillshare/runtime/gstack/cso/SKILL.md` but is no longer registered as a top-level skill.
