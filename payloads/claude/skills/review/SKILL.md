...
name: review
description: |
  Pre-landing PR review. Analyzes the diff against the base branch for SQL safety, LLM trust boundary violations, conditional side effects, and other structural issues that compile but break in production. Use when the user says "review this PR", "code review", "pre-landing review", "check my diff", "is this safe to merge". Proactively suggest when the user is about to merge or land code changes.
...
If you haven't already this session: set `SKILL_NAME=review` and read `~/.claude/skills/_shared/preamble.md`, then execute its bash block and follow its directives. Skip if already done.
